...
for i = 1:length(ind_command)
    ...
    switch cmd_use
        ...
        case 'motor_cam_s6bmb'
            ...
            edd(i).log       = snlog(1:lf(11));
            edd(i).motorname = mname;
            edd(i).motor_start_end_numstep = [mstart mend numstep];
            edd(i).motorstep = (mend-mstart)/numstep;
            edd(i).exp_time  = exptime;
            % edd(i).data      = zeros(nosteps,2048);
    ...
    if nosteps == 0
        ...
        switch cmd_use
            case 'motor_cam'
                edd(i).data{2} = zeros(nosteps,8192);
            case 'motor_cam2'
                edd(i).data{2} = zeros(nosteps,8192);
            case 'motor_cam_s6bmb'
                edd(i).data{2} = zeros(nosteps,2048);
        end
    else
        for j = 1:1:nosteps
            ...
            switch cmd_use
                ...
                case 'motor_cam2'
                    numcol = length(da)/8192;              % number of column
                case 'motor_cam_s6bmb'
                    numcol = length(da)/2048;              % number of column
            end
