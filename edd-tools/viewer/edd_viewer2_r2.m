...
if isempty(heddviewMain)
    ...
    % --- Folder/File path(s)
    % --------------------------------
    config.folder   = '';  % wording directory
    config.file     = '';  % 
...
hcopyright = text('Parent',hAxes,...
    ...
    'fontsize',8,...
    'String',{'by AC@APS, ANL','Ver.1.3.1 January. 2018'},...
  ...
  function visopt_Callback(hObject,~)
      ...
      switch tag
          ...
          case 'visopt_push_forward'
              if isfield(h,'data')
                  size(h.data(h.visopt_pop_scno.Value).data,1);
                  posno = str2num(h.visopt_edit_xpos.String);
                  if max(posno) < size(h.data(h.visopt_pop_scno.Value).data,1)
          ...
          case 'visopt_pop_scno'
              ...
              if isempty(h.data(scno).data)
                  h.visopt_chkbox_det1.Value = 0;
                  h.visopt_chkbox_det1.Enable = 'off';
              else
                  h.visopt_chkbox_det1.Enable = 'on';
              end
              if isempty(h.data(scno).data2)
                  h.visopt_chkbox_det2.Value = 0;
                  h.visopt_chkbox_det2.Enable = 'off';
              else
                  h.visopt_chkbox_det2.Enable = 'on';
...
function main_calibrate_Callback(~,~)
      %warndlg({'Calibration GUI is a paid function!!','Please upgrade to the full version.'},'!! Warning !!')
      h = get_handle;
      edd_calibrant(h);
...
%       hfig = findall(0,'Tag','eddview_det1_2d');
%       if strcmp(get(hfig,'Type'),'figure')
%           %ax1 = get(hfig,'CurrentAxes');
%           %ax1.Children.CData = log10(da(1).data);
%           opt.fig_handle = hfig;
%           eddplot_mar17(da,opt);
%       else
%           fig1 = figure(101);
%           set(fig1,'Tag','eddview_det1_2d','Position',[400 600 800 600]);
%           opt.fig_handle = fig1;
%           eddplot_mar17(da,opt);
%           %imagesc(log10(da(1).data))
%       end
      %heddviewdet1 = findall(0,'Tag','eddviewmain_Fig');
      %if isempty(heddviewMain)
