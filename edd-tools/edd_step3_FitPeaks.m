...
for i = 1:1:numDV
    ...
    for j = 1:1:length(pkid_fit)
        ...
        switch lower(pkfitting_pars.pfunc_type)
            case 'pseudovoigt'
                p0  = [ max(ydata); 0.5; 0.01;     E_hkl(pkid_fit(j));     0;   0; ];
                pLB = [          0;   0;    0; E_hkl(pkid_fit(j)) - 3; -inf; -inf; ];
                pUB = [        inf; inf;    1; E_hkl(pkid_fit(j)) + 3;  inf;  inf; ];
            case 'splitpseudovoigt'
                p0  = [ max(ydata); 0.5; 0.5; 0.01; 0.01;     E_hkl(pkid_fit(j));    0;    0; ];
                pLB = [          0;   0;   0;    0;    0; E_hkl(pkid_fit(j)) - 3; -inf; -inf; ];
                pUB = [        inf; inf; inf;    1;    1; E_hkl(pkid_fit(j)) + 3; +inf; +inf; ];
            otherwise
                disp('Unknown peak function!!')
                return
        end
        pkfitting_pars.xdata    = xdata;
        [p, rn(j), ~, ef(j)]    = lsqcurvefit(@pfunc_switch, p0, pkfitting_pars, ydata, pLB, pUB);
