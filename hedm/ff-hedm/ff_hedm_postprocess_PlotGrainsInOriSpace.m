...
scatter3(rod(1,:), rod(2,:), rod(3,:), 30, 'b')
axis equal tight off
title('Orientations of found grains')

% PLOT ORIENTATIONS / COMPLETENESS
figure,
PlotFRPerimeter('hexagonal');
scatter3(rod(1,:), rod(2,:), rod(3,:), 30, cidx, 'filled')
axis equal tight off; colormap jet;
colorbar vert; caxis([0.5 1.0])
title('Orientations of found grains // color denotes completeness')

% PLOT ORIENTATIONS / VM
figure,
PlotFRPerimeter('hexagonal');
scatter3(rod(1,:), rod(2,:), rod(3,:), 30, vm, 'filled')
axis equal tight off; colormap jet;
colorbar vert; caxis([min(vm) max(vm)])
title('Orientations of found grains // color denotes vm')

% PLOT ORIENTATIONS / h
figure,
PlotFRPerimeter('hexagonal');
scatter3(rod(1,:), rod(2,:), rod(3,:), 30, h, 'filled')
axis equal tight off; colormap jet;
colorbar vert; caxis([min(h) max(h)])
title('Orientations of found grains // color denotes h')
...
for i = 1:1:6
    ...
    figure,
    PlotFRPerimeter('hexagonal');
    scatter3(rod(1,:), rod(2,:), rod(3,:), 30, sij, 'filled')
    axis equal tight off; colormap jet;
    colorbar vert; caxis([min(sij) max(sij)])
    title('Orientations of found grains // color denotes sij')
end
