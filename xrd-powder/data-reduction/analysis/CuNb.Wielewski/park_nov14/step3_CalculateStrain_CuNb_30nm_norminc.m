...
XRDIMAGE.DarkField.fbase    = 'CuNb_30nm_';
XRDIMAGE.DarkField.fnumber  = 9047;
...
XRDIMAGE.Image.fnumber      = [9041:9046]; % 50nm: 392 : 424 % 30 nm : 351; 390
...
%                             pkfit{ii}.tth(XRDIMAGE.Material.pkidx{ii,jj},k)
                            ...
                            eqq = sind(th0) / sind(th) - 1;
                            LatticeStrain{i,ii}(XRDIMAGE.Material.pkidx{ii,jj},k)   = eqq;
                            Amplitude{i,ii}(XRDIMAGE.Material.pkidx{ii,jj},k)       = pkfit{ii}.amp(XRDIMAGE.Material.pkidx{ii,jj},k);
                        end
                    end
                end
            end
            ...
            for ii = 1:1:XRDIMAGE.Material.num
                for jj = 1:1:size(XRDIMAGE.Material.pkrange,2)
                    if ~isempty(XRDIMAGE.Material.pkidx{ii,jj})
                        ...
                        subplot(2,2,3)
                        scatter(XRDIMAGE.CakePrms.azim(:), LatticeStrain{i,ii}(XRDIMAGE.Material.pkidx{ii,jj},:), ...
                            30, Amp(:))
                        title(sprintf('%s \\{%d %d %d\\}', MatStr, pkfit{ii}.hkl{XRDIMAGE.Material.pkidx{ii,jj},1}))
                        hold on
...
AveLatticeStrain{1} = (LatticeStrain{1,1} + LatticeStrain{2,1} + LatticeStrain{3,1} + LatticeStrain{4,1} + LatticeStrain{5,1})./5;
AveLatticeStrain{2} = (LatticeStrain{1,2} + LatticeStrain{2,2} + LatticeStrain{3,2} + LatticeStrain{4,2} + LatticeStrain{5,2})./5;

figure(10001)
plot((1:1:XRDIMAGE.CakePrms.bins(1)).*10, AveLatticeStrain{1}(1,:), 'ko')
hold on
plot((1:1:XRDIMAGE.CakePrms.bins(1)).*10, AveLatticeStrain{1}(2,:), 'ro')
xlabel('azimuth (deg)')
ylabel('strain')
grid on
axis([0 350 -5e-3 5e-3])
legend('Cu111', 'Cu200')

figure(10002)
plot((1:1:XRDIMAGE.CakePrms.bins(1)).*10, AveLatticeStrain{2}(1,:), 'ko')
hold on
plot((1:1:XRDIMAGE.CakePrms.bins(1)).*10, AveLatticeStrain{2}(2,:), 'ro')
plot((1:1:XRDIMAGE.CakePrms.bins(1)).*10, AveLatticeStrain{2}(3,:), 'bo')
xlabel('azimuth (deg)')
ylabel('strain')
grid on
axis([0 350 -5e-3 5e-3])
legend('Nb110', 'Nb200', 'Nb211')