function mappedData = XFormRadialSpectraPeaks(centTrans, D, gammaYprime, gammaXhatPrime, data, azim, tthRange,DistPars,DistType)
...
for i = 1:numSpectra
    ...
    if DistType==0
        rhoHatdPrime= data(:,i);
    elseif DistType==1
        rhoHatdPrime  = data(:,i) + DistPars(1);
    elseif DistType==2
        f = DistPars(1)*((data(:,i)/DistPars(7)).^(DistPars(3)))*cosd(DistPars(5)*azim(i)+DistPars(6))...
            + DistPars(2)*((data(:,i)/DistPars(7)).^DistPars(4)) + 1;
    ...
    %% transform data
    %     tmpData = [rad2deg(measTTH(:)), etaHatPrime(:)];
