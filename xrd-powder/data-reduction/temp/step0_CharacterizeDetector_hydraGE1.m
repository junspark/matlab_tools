...
% fname   = 'Ceo2_calibr1_00029.ge1.sum';
% pfname  = fullfile(pname, fname);
% 
% data    = ReadSUM(pfname);
% data    = imrotate(data, 152.5);
% data    = [zeros(500, size(data,2)); data];
% data    = [zeros(size(data,1), 250) data zeros(size(data,1), 250)];
% imagesc(log(data))
% axis equal tight
% return
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
XRDIMAGE.Instr.detectorsize = 3265*XRDIMAGE.Instr.pixelsize;    % mm
XRDIMAGE.Instr.distance     = 1875.359;     % mm
...
%%% CAKE PARAMETERS
% XRDIMAGE.CakePrms.bins(1)   = 3;            % number of azimuthal bins  %% 72
% XRDIMAGE.CakePrms.bins(2)   = 2000;         % number of radial bins
% XRDIMAGE.CakePrms.bins(3)   = 7;            % number of angular bins
% XRDIMAGE.CakePrms.origin(1) = 2266.089;     % x center in pixels, fit2d Y 
% XRDIMAGE.CakePrms.origin(2) = 2134.818;     % y center in pixels, fit2d X
% XRDIMAGE.CakePrms.sector(1) = 180;          % start azimuth (min edge of bin) in degrees    %% -360/XRDIMAGE.CakePrms.bins(1)/2
% XRDIMAGE.CakePrms.sector(2) = 250;          % stop  azimuth (max edge of bin) in degrees   %% 360-360/XRDIMAGE.CakePrms.bins(1)/2
% XRDIMAGE.CakePrms.sector(3) = 650;          % start radius (min edge of bin) in pixels
% XRDIMAGE.CakePrms.sector(4) = 1500;         % stop  radius (max edge of bin) in pixels

XRDIMAGE.CakePrms.bins(1)   = 3;            % number of azimuthal bins
XRDIMAGE.CakePrms.bins(2)   = 5;            % number of radial bins
XRDIMAGE.CakePrms.bins(3)   = 7;            % number of angular bins
XRDIMAGE.CakePrms.origin(1) = 1213;           % x center in pixels, 
XRDIMAGE.CakePrms.origin(2) = 72;         % y center in pixels, 
XRDIMAGE.CakePrms.sector(1) = 80;          % start azimuth (min edge of bin) in degrees    %% -360/XRDIMAGE.CakePrms.bins(1)/2
XRDIMAGE.CakePrms.sector(2) = 100;          % stop  azimuth (max edge of bin) in degrees   %% 360-360/XRDIMAGE.CakePrms.bins(1)/2
XRDIMAGE.CakePrms.sector(3) = 650;          % start radius (min edge of bin) in pixels
XRDIMAGE.CakePrms.sector(4) = 660;          % stop  radius (max edge of bin) in pixels
...
XRDIMAGE.Material.lattparms = 5.411651;        % LaB6
XRDIMAGE.Material.structure = 'fcc';
XRDIMAGE.Material.numpk     = 6;
XRDIMAGE.Material.pkrange    = [...
    3.1 3.65 5.2 6.1 6.3 7.4; ...
    3.3 3.85 5.4 6.3 6.6 7.6; ...
    ];
...
if Analysis_Options.make_polimg
    for i = 1:1:numimg
        ...
        imgi    = [zeros(500, size(imgi,2)); imgi];
        imgi    = [zeros(size(imgi,1), 250) imgi zeros(size(imgi,1), 250)];
        ...
        ylabel('Y_{L}')
        hold on
        plot(XRDIMAGE.CakePrms.origin(1), XRDIMAGE.CakePrms.origin(2), 'r.')
