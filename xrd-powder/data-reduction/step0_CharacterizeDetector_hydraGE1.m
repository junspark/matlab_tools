...
% data    = [zeros(1040, size(data,2)); data];
...
XRDIMAGE.Instr.distance     = 1875.743263;     % mm
XRDIMAGE.Instr.centers      = [ -860.221891 , 530.439567 ]; % center offsets x & y (um)
XRDIMAGE.Instr.gammaX       = -0.005784;    % rad
...
XRDIMAGE.CakePrms.origin(1) = 1483;         % x center in pixels, 
XRDIMAGE.CakePrms.origin(2) = 812;          % y center in pixels, 
XRDIMAGE.CakePrms.sector(1) = 50;           % start azimuth (min edge of bin) in degrees    %% -360/XRDIMAGE.CakePrms.bins(1)/2
XRDIMAGE.CakePrms.sector(2) = 110;          % stop  azimuth (max edge of bin) in degrees   %% 360-360/XRDIMAGE.CakePrms.bins(1)/2
...
if Analysis_Options.make_polimg
    for i = 1:1:numimg
        ...
        imgi    = [zeros(1040, size(imgi,2)); imgi];
