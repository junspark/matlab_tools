...
Material.d_spacing_UB   = d_spacing_UB;
Material.d_spacing_LB   = d_spacing_LB;
Material.pkidx      = {...
    [1] [2] [3]
    };
...
for iiii = 1:1:length(FROOT)
    ...
    switch XRDIMAGE.Image.corrected
        case 'sum'
            XRDIMAGaE.Image.scan_nframes = 1;
        case {'cor', 'cor32'}
    ...
    if Analysis_Options.fits_spectra
        for iii = 1:1:numimg
            ...
            parfor jjj = 1:1:XRDIMAGE.Image.scan_nframes(iii)
                tic
                FitPeaksPerPolImage(pfname{iii, jjj}, Material, Analysis_Options)
                toc
                return
            end
