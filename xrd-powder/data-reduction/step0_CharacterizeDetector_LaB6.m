...
XRDIMAGE.Material.lattparms = 4.156918;
...
XRDIMAGE.Material.numpk     = 11;
XRDIMAGE.Material.numbounds = 11;
...
for iii = 1:1:XRDIMAGE.Material.numbounds
    XRDIMAGE.Material.pkrange(:,iii)  = [ ...
        ...
        max(tth_UB(XRDIMAGE.Material.pkidx{iii})); ...
        ];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iii = 1:1:numimg
    ...
    pfname{iii, 1}  = fullfile(XRDIMAGE.Image.pname, ...
        XRDIMAGE.Image.froot, XRDIMAGE.Image.fext, fname);
end
...
if Analysis_Options.make_polimg
    for iii = 1:1:numimg
        disp('###########################')
        disp(sprintf('Looking at %s', pfname{iii,1}))
        disp('###########################')
        
        pfname_polimage = sprintf('%s.polimg.mat', pfname{iii,1});
        ...
        if Analysis_Options.save_polimg
            disp(sprintf('Saving polimg for %s', pfname{iii,1}))
            save(pfname_polimage, 'polimg', 'XRDIMAGE')
        else
            disp(sprintf('Not saving polimg for %s', pfname{iii,1}))
        end
...
if Analysis_Options.fits_spectra
    for iii = 1:1:numimg
        pfname_polimage = [pfname{iii,1}, '.polimg.mat'];
        pfname_pkfit    = [pfname{iii,1}, '.pkfit.mat'];
...
if Analysis_Options.find_instrpars
    for iii = 1:1:numimg
        ...
        disp('###########################')
        disp(sprintf('Looking at %s to find instrument parameters.', pfname{iii,1}))
        disp('###########################')
