function polImg = reBin(mesh,instr,cakeParms,imageData)
% imageData   = imageData';
% img         = imageData(:,sort(1:size(imageData,2),'descend'));
% 
% Ifunc   = reshape(img,size(imageData,2)*size(imageData,2),1);%imageData(crd(1,:),crd(2,:));
% % Ifunc   = 2^16-double(Ifunc);
Ifunc   = double(imageData);
