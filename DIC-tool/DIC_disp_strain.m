...
for i = 2:1:length(fnum)
    ...
    % udiff   = mean((A*uCoef - disp_h)./disp_h);
    % vdiff   = mean((A*vCoef - disp_v)./disp_v);
    ...
    % imgb    = imadjust(imdata_curr_zoomed,[0 1],[0.5 0.9]);
    % h1      = imshow(imdata_curr_zoomed,'InitialMagnification', 'fit');
    imagesc(imdata_curr_zoomed)
    axis equal tight on
    colormap(gray)
    hold on;
    ...
    % Make axis2 as large as axis1
    h2 = quiver(hc,vc,disp_h,disp_v);
    set(ax2,'color','none');
    % Make quiver plot background transparent
