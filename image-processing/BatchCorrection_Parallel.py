...
def ReadImage(pfname_ge, darkvalues, badInd, badInd1, ndel, outpath, num_X, num_Y):
  import numpy
  import os

  fread = numpy.fromfile
  ### pfname_ge, darkvalues, badInd, badInd1, ndel, outpath, num_X, num_Y = inputs
  
  sumvalues = numpy.zeros(num_X*num_Y,numpy.float32)
  
  statinfo = os.stat(pfname_ge)
  ...
  (d, fout) = os.path.split(pfname_ge)
  with open(pfname_ge, mode='rb') as fileobj:
    fileobj.seek(8192)
    for i in range(nFrames):
      ...
      if ndel:
        ...
        print '\nSaving frame number ' + str(i) + ' to ' + frameName
...
parser.add_argument('--nnode', type=int, nargs=1, default=[1], help='GE number. Default = 1.')
...
# START IMAGE CORRECTION

### CREATE CLUSTER FOR PP TO WORK WITH
# tuple of all parallel python servers to connect with
ppservers = ()
ppservers = ("10.0.0.1",)
ncpus = int(clargs.nnode[0])
job_server = pp.Server(ncpus, ppservers=ppservers)

## Regular run by calling function ReadImage
##start_time = time.time()
##ReadImage(files[0], darkvalues, badInd, badInd1, clargs.ndel, outpath, num_X, num_Y)
##print "******************* STATS *******************"
##print "Time elapsed: ", time.time() - start_time, "s"
##print "*********************************************"

# tuple input for 1 node / 1 image case
inputs = files[0], darkvalues, badInd, badInd1, clargs.ndel, outpath, num_X, num_Y

start_time = time.time()
jobs = job_server.submit(ReadImage, inputs,)
jobs.wait()
print "******************* STATS *******************"
print "Time elapsed: ", time.time() - start_time, "s"
print "*********************************************"
...
# tuple input for all node (number of nodes specified) / all images case
job_server = pp.Server(ncpus, ppservers=ppservers)
jobs = []
start_time = time.time()
for i in range(len(files)):
  iinputs = files[i], darkvalues, badInd, badInd1, clargs.ndel, outpath, num_X, num_Y
  jobs.append(job_server.submit(ReadImage, iinputs))
